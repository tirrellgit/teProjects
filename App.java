...
public class App {
    ...
    private void initialize() {
        ...
        String memberFileDataPath = "src/main/resources/members.dat";
        List<String> memberInfo = new ArrayList<>();

        try {
            memberInfo = FileStorageService.readContentsOfFile(memberFileDataPath);

        } catch (FileStorageException e) {
            throw new RuntimeException(e);
        }
        for (String memberLine : memberInfo) {
            String[] singleMemberData = memberLine.split(FIELD_DELIMITER);
            String firstName = singleMemberData[0];
            String lastName = singleMemberData[1];
            String fileName = singleMemberData[2];
            Member member = new Member(firstName, lastName);

            String dataFileName = FILE_BASE_PATH + fileName;
            List<CatalogItem> memberItems = processMemberItems(dataFileName);
    ...
    private List<CatalogItem> processMemberItems(String dataFileName) {
        // return all CatalogItems for a single user
        List<CatalogItem> singleMemberItems = new ArrayList<>();

        //use FileStorageService to get a list of strings, each of which represents one CatalogItem
        List<String> catalogItemInfo = new ArrayList<>();
        try {
            catalogItemInfo = FileStorageService.readContentsOfFile(dataFileName);
        } catch (FileStorageException e) {
            System.out.println("File Storage Exception");
        }
        for (String singleCatalogItem : catalogItemInfo) {
            //break each line into it's component parts (catalog type, fields for catalog item except id)
            String[] singleItem = singleCatalogItem.split(FIELD_DELIMITER);
            if (singleItem[0].equals("book")) {
                // its a book
                // call on book constructor
                // register item
                // add finished item to SingleMemberItems
                String title = singleItem[1];
                String author = singleItem[2];
                LocalDate publishDate = LocalDate.parse(singleItem[3]);
                Book book = new Book(title, author, publishDate);
                book.registerItem();
                singleMemberItems.add(book);

            } else if (singleItem[0].equals("movie")) {
                // its a movie
                // call on book constructor
                // register item
                // add finished item to SingleMemberItems
                String name = singleItem[1];
                String director = singleItem[2];
                LocalDate publishDate = LocalDate.parse(singleItem[3]);
                Movie movie = new Movie(name, director, publishDate);
                movie.registerItem();
                singleMemberItems.add(movie);

            } else if (singleItem[0].equals("tool")) {
                // its a tool
                // call on book constructor
                // register item
                // add finished item to SingleMemberItems
                String type = singleItem[1];
                String manufacturer = singleItem[2];
                int count = Integer.parseInt(singleItem[3]);
                Tool tool = new Tool(type, manufacturer, count);
                tool.registerItem();
                singleMemberItems.add(tool);

            } else {
                System.err.println("Items not entered correctly");
            }
        }


        return singleMemberItems;
    }
